...
- **New convention document** `ai/capability-matrix.md` — the canonical matrix format: columns, ID scheme, verb-with-object rule, eval-before-status rule, `ok`/`wanted` statuses, row ownership, lifecycle. Routed from `ai/index.md`.
- **`ai/tech-approach.md`** — the matrix joins "What to Include"; the testing exclusion narrows from all testing strategies to free-text test plans only.
- **`ai/workflow.md`** — refine-flow steps for the technical approach and phase documents gain matrix authoring and capability-ID acceptance criteria.
...
| `obs.xref` | Observe that every relative doc link under `ai/` resolves to an existing file | `cd ai && grep -hoE '\(\./[a-z-]+\.md\)' *.md \| tr -d '()' \| sort -u \| xargs -r -I{} test -e {}` exits 0 | ok |
| `obs.index` | Observe that every `ai/` document is routed from `ai/index.md` | `for f in ai/*.md; do grep -q "$(basename "$f")" ai/index.md \|\| exit 1; done` exits 0 | ok |
