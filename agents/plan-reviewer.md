...
- Every eval is runnable from the row text alone with a binary outcome. You do not prove determinism — you check that the row names a concrete invocation and pass condition (exit 0, endpoint answers, suite passes), then run each `ok` row's eval that is plainly read-only (from the repo root, unless the row states otherwise) to confirm it passes. Report a row whose eval fails (the status lies) or whose eval can't be run from the row text alone (the row is underspecified). If an eval would mutate state, flag that as a finding instead of running it.
